...
object CompilerIntegrationSpec extends BaseSpec {
  ...
  def runTest(webpackConfigFilename: String) = {
    ...
    val compiler = new Compiler(
      ...
      webpackConfig = new File("src") / "test" / "scala" / "givers" / "vuefy" / "assets" / webpackConfigFilename,
...
//            result.entries.head.filesRead ==> inputs.map(_.toPath).toSet ++ Set(baseInputDir / "dependencies" / "style.scss")
  ...
  val tests = Tests {
    'compile - {
      ...
      "run webpack and get result correctly (webpack.func-style.config.js)" - {
        runTest("webpack.func-style.config.js")
      }
    }
  }
}
